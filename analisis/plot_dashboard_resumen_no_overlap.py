...
FIGS = BASE / "outputs" / "figures_no_overlap"
...
def panel_c(ax) -> None:
    ...
    ax.bar(x + width / 2, composition, width, label="Composici\u00f3n",
...
def panel_d(ax) -> None:
    ...
    x_max = 0.85
    for i, (yp, b, hi) in enumerate(zip(y_pos, betas, ci_hi)):
        ax.scatter(b, yp, color=C_GRADIENT[i], s=70, zorder=3,
                   edgecolors="white", linewidth=1.2)
        x_text = min(hi + 0.01, x_max - 0.005)
        ha = "right" if x_text >= x_max - 0.006 else "left"
        y_off = -6 if i % 2 == 0 else 6
        ax.annotate(
            f"{b:.0%}",
            xy=(x_text, yp),
            xytext=(0, y_off),
...
def main() -> None:
    ...
    fig.savefig(FIGS / "dashboard_resumen_latex.png", dpi=300, bbox_inches="tight", pad_inches=0.1)
    ...
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    fig.savefig(FIGS / "dashboard_resumen_linkedin.png", dpi=100, bbox_inches="tight", pad_inches=0.18)
