...
Version "no_overlap": ajustes de layout para evitar superposiciones
de titulos/etiquetas/annotations en exportaciones LaTeX y LinkedIn.

Genera 3 figuras:
  1. Coefficient Plot (Part A): year dummies en ALWAYS_IN
  2. Parallel Trends: tasas cumple_v4 por grupo y a\u00f1o
...
def save_dual(fig, name: str) -> None:
    ...
    # LaTeX
    fig.savefig(FIGS / f"{name}_latex.png", dpi=300, bbox_inches="tight", pad_inches=0.08)
    # LinkedIn (1200x630)
    ...
    fig.savefig(FIGS / f"{name}_linkedin.png", dpi=100, bbox_inches="tight", pad_inches=0.12)
    plt.close(fig)


def set_y_padding(ax, values, frac: float = 0.18, min_pad: float = 0.03) -> None:
    """Agrega margen vertical para evitar solapes con annotations."""
    ymin = min(values)
    ymax = max(values)
    span = ymax - ymin
...
def plot_coeff_part_a() -> None:
    ...
    # Annotations
    offset_y = -18 if betas[3] > 0 else 18
    offset_x = 10
    ax.annotate(
        f"+{betas[3]:.1%}",
        xy=(2025, betas[3]),
        ...
        color=C_MAIN,
        ha="left",
        va="top" if betas[3] > 0 else "bottom",
...
def plot_parallel_trends() -> None:
    """Tasas cumple_v4 por grupo y a\u00f1o."""
...
def plot_coeff_part_b() -> None:
    ...
    # Annotation for 2025 (use offset points to avoid x-label overlap)
    offset_y = 20 if betas[3] < 0 else -20
    offset_x = -18 if betas[3] < 0 else 18
    ax.annotate(
        f"{betas[3]:+.1%}\n(SWITCHER < ALWAYS_IN)",
        xy=(2025, betas[3]),
        xytext=(offset_x, offset_y),
        ...
        color=C_ACCENT,
        fontweight="bold",
        ha="right" if betas[3] < 0 else "left",
        va="bottom" if betas[3] < 0 else "top",
