"""
Gr\u00e1ficos Heterogeneidad PIA: efecto diferencial por tama\u00f1o presupuestal.
...
def plot_quintile_effects() -> None:
    ...
    # Value annotations (offset to avoid overlap with markers/CI)
    x_max = 0.87
    for i, (yp, b, s, hi) in enumerate(zip(y_pos, betas, ses, ci_hi)):
        t_stat = b / s
        sig = "***" if abs(t_stat) > 2.576 else "**" if abs(t_stat) > 1.96 else ""
        x_text = min(hi + 0.012, x_max - 0.005)
        ha = "right" if x_text >= x_max - 0.006 else "left"
        y_off = -10 if i % 2 == 0 else 10
        ax.annotate(
            f"{b:.1%}{sig}",
            xy=(x_text, yp),
            xytext=(0, y_off),
...
def plot_interactions() -> None:
    ...
    ax.text(0.02, 0.04, f"Efecto base Q1:\n+{base_beta:.1%}",
            ha="left", va="bottom", fontsize=9, color=C_MAIN, fontweight="bold",
